/**
 * V1: the original multi-section marketing site.
 *
 * ===========================================================================
 * RESTORED FROM THE ARCHIVE, NOT REWRITTEN
 * ===========================================================================
 * This is the page that was at `/` until 2026-07-30, lifted verbatim from
 * `homepage-archive-2026-07-30` (also the branch archive/homepage-2026-07-30).
 * Only two things changed: the `metadata` export moved to lib/site-version.ts,
 * where all three versions' metadata now lives, and the component was renamed
 * from a page default export to a named one.
 *
 * Every section it imports was still in the tree, unchanged, which is what
 * made this a move rather than a rebuild. That was checked rather than assumed
 * before the restore: all twelve of smart-fart-hero, quiz-nav, steps,
 * comparison, feature-grid, testimonials, faq, waitlist, video-showcase,
 * section, heading and section-divider resolve.
 *
 * It renders its OWN <SiteFooter />, because the footer left the root layout
 * when v3 arrived (see app/(site)/layout.tsx) and a long scrolling marketing
 * page is exactly what that footer's overlapping wave was designed for.
 *
 * Reached by setting SITE_VERSION=v1, or `?v=1` in development.
 */
  ...
  return (
    <>
      {/*
        ...
        scroll-depth and section-view tracking on. The archived original did not
        have it — it predates the marker, and was keyed to a hero class name
        that has since been deleted, which is how tracking silently died on
        2026-07-30. Added on restore so v1's analytics work.
      */}
      ...
      {/* v1 brings its own footer: it left the root layout when v3 arrived, and
          a long scrolling page is what its overlapping wave was built for. */}
      <SiteFooter />
    </>
  );
}
