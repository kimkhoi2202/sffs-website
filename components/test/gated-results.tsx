 ...
 *
 * ===========================================================================
 * THE IN-PLACE REVEAL WAS TRIED, AND TAKEN BACK OUT
 * ===========================================================================
 * For part of one afternoon a successful send unblurred this screen: verdict,
 * score, question review and the share controls, all in place. It is written
 * down because the code no longer shows it and the argument for it was a real
 * one — a mistyped address left somebody with nothing.
 *
 * It was reverted deliberately. A score on this screen leaves the email with
 * no job, so nobody opens it, and the inbox is the thing this whole flow is
 * built to reach. The recovery case it was solving is covered instead by
 * ./saved-result-offer.tsx, which hands the SAME emailed link back to a
 * returning visitor rather than printing the score here.
 *
 * So: do not reintroduce a reveal on this screen. The one route to a score is
 * /results/[token], and the one route to that is a link.
 *
 * ===========================================================================
...
export interface GatedResultsProps {
  ...
  token: string | null;
  /**
   * A message has genuinely left. It does NOT unlock anything on this screen;
   * the caller uses it to remember the emailed link in this browser, so a
