...
- Everyday scenarios where the intuitive answer is wrong but the real answer is simple once seen.
- Deliver as a text card or MC (A–D); one unambiguous answer + a one-line "of course!" reveal.
- Keep them fair (not trick-for-trick's-sake), age-appropriate, and solvable from one screen.
- Coordinate the "X% fail" framing with the difficulty-calibrator so it stays honest.

Invent every question — never reuse a source video's item.
...
- [ ] Each has one unambiguous answer + an "of course!" reveal.
- [ ] "X% fail" framing coordinated with the calibrator; one screen each.
