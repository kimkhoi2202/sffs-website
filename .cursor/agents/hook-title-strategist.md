...
- A **video title** + 3–5 alternates in the "X% fail" / "only a genius can…" shape — original wording, honest framing (no unverifiable superlatives).
- A **first 1–2 second hook** (spoken + on-screen) that sets the stakes fast.
- **Thumbnail text** (≤6 words, one boxed word) + a layout note using DESIGN.md Recipe A/B.
- **9:16 + carousel** hook variants.

Coordinate with the difficulty-calibrator so any "X%" number is defensible; never copy a source video's title or hook.

## Inputs / outputs
- Inputs: the video brief + puzzle mix.
...
- [ ] Hook lands in 1–2s; any "X%" framing is honest and calibrator-backed.
- [ ] Thumbnail text ≤6 words, on-brand, legible at 168×94.
- [ ] 9:16 + carousel hook variants provided.
