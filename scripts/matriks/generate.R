...
# ---------------------------------------------------------------------------
# DIFFICULTY RAMPS WITHIN EACH BANK, IT DOES NOT SIT FLAT.
#
# The first version of these specs gave every matrix in a bank the same number
# of simultaneous rules, on the reasoning that the band table in the taxonomy
# names one figure for each band. That produced five variations on one idea and
# nothing that separated the child who has got it from the child who has not.
# The ladder below is per bank: the first matrix is the accessible one, the last
# two are the ones that do the separating.
#
#   Grade 3    1, 1, 1, 2, 2      (three of these come from the local engine in
#                                  build-figures.mjs, which owns shape identity
#                                  and element count)
#   Grade 4    1, 2, 2, 2, 2
#   Grade 5    2, 2, 2, 2, 3
#   Grade 6    2, 2, 3, 3, 3
#   Grades 7-8 3, 3, 3, logic, logic
#
# GRADE 3 IS CAPPED AT TWO, deliberately. Two composed rules is the target for
# its final items and three would be overshooting: the taxonomy is explicit that
# difficulty comes from composition rather than from obscurity, and an
# eight-year-old holding three relations at once under a twenty-second clock is
# measuring working memory rather than reasoning.
#
# GRADE 3 AND THREE OF GRADE 4'S MATRICES ARE NOT HERE. They use shape identity
# and element count, which matRiks cannot express in this glyph set, and they
# are generated by the second engine in build-figures.mjs. See the note there.
specs <- list(
  ...
  list(id = "g5-m5", base = c("triangle", "square"),         h = c("rotate", "size"), v = "shade", glyphs = c("triangle", "teardrop"),    rot = 2),

  # ---- Grade 6 (L12): two, two, then three, three, three ------------------
  list(id = "g6-m1", base = c("triangle"),                   h = "rotate", v = "size",     glyphs = c("teardrop"),                        rot = 1),
  list(id = "g6-m2", base = c("square"),                     h = "rotate", v = "shade",    glyphs = c("arrow"),                           rot = 2),
  ...
  # only a render can.
  list(id = "g6-m4", base = c("pentagon"),                   h = c("shade", "size"), v = "rotate", glyphs = c("heart"),                   rot = 1),
  list(id = "g6-m5", base = c("triangle", "circle"),         h = c("size", "rotate"), v = "shade", glyphs = c("triangle", "arrow"),       rot = 2),

  # ---- Grades 7-8 (L13/14): three rules, then the logical operators -------
  list(id = "g78-m1", base = c("square"),                    h = c("shade", "size"), v = "rotate", glyphs = c("heart"),                   rot = 2),
  list(id = "g78-m2", base = c("triangle", "circle"),        h = c("rotate", "size"), v = "shade", glyphs = c("arrow", "teardrop"),       rot = 1),
  list(id = "g78-m3", base = c("pentagon"),                  h = c("shade", "rotate"), v = "size", glyphs = c("teardrop"),                rot = 1),
  list(id = "g78-m4", base = c("circle", "square", "triangle", "pentagon"), h = "XOR", v = "identity", glyphs = c("circle", "square", "triangle", "star"), rot = 1),
  list(id = "g78-m5", base = c("circle", "square", "triangle", "pentagon"), h = "AND", v = "identity", glyphs = c("heart", "cross", "diamond", "lightning"), rot = 1),
